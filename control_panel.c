...
void mostrar_estado_sistema()
{
    ...
    mvwprintw(win_status, 4, 2, "[SEL] Banda seleccionada: %d", banda_seleccionada + 1);
    if (has_colors())
...
void pausar_reanudar_banda(int banda_id)
{
    if (banda_id >= 0 && banda_id < datos_compartidos->num_bandas)
    {
        ...
        if (banda->pausada)
        {
            ...
            mostrar_mensaje_temporal("[OK] Banda REANUDADA");
        }
        else
        {
            banda->pausada = 1;
            mostrar_mensaje_temporal("[PAUSE] Banda PAUSADA");
        }
    }
}
...
void mostrar_ayuda_detallada()
{
    ...
    mvprintw(7, 5, "|   ^/v              Cambiar banda seleccionada                                  |");
    ...
    mvprintw(17, 5, "|   Vista General    Resumen de todas las bandas y estadisticas                |");
    mvprintw(18, 5, "|   Detalle Banda    Informacion especifica de la banda seleccionada           |");
    mvprintw(19, 5, "|   Inventario Global Resumen de inventarios por ingrediente                   |");
    mvprintw(20, 5, "|                                                                                |");
    mvprintw(21, 5, "| INDICADORES:                                                                   |");
    mvprintw(22, 5, "|   [OK] Verde         Funcionamiento normal                                       |");
    mvprintw(23, 5, "|   [!]  Amarillo      Advertencia/Critico                                        |");
    mvprintw(24, 5, "|   [X]  Rojo          Error/Agotado/Inactivo                                       |");
    mvprintw(25, 5, "|                                                                                |");
...
int main()
{
    ...
    if (!datos_compartidos->sistema_activo)
    {
        printf("El sistema principal no esta activo\n");
        printf("   Inicia primero: ./burger_system -n 4\n");
        exit(1);
    }

    printf("Iniciando interfaz de control...\n");
    sleep(1);
    ...
    while ((ch = getch()) != 'q' && ch != 'Q')
    {
        ...
        if (ahora - ultimo_refresh >= 1 || ch != ERR)
        {
            ...
            // Mostrar interfaces segun el modo
            switch (modo_vista)
            {
